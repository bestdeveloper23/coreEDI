...
export class ProductService extends BaseService {
    ...
    getProductRecentTransactionsAll(dateFrom, dateTo, ignoreDate, limitcount, limitfrom, productID,deleteFilter) {
        let options = {};
        if (productID!="all"){
        if (ignoreDate == true) {
          options = {
            IgnoreDates: true,
            OrderBy: "TransactionNo desc",
            PropertyList: "TransactionDate,ProductName,FirstColumn,SecondColumn,ThirdColumn,Qty,TotalCost,ProductID,ClassID,TransactionNo,AverageCost,Cost,Available,InStock,so,invbo,pobo,onbuild,building,Price,TotalPrice,ExtraDesc,TranstypeDesc,Deptname,Active",
    ...
    getProductSalesOrder(dateFrom, dateTo, ignoreDate, limitcount, limitfrom, productID,deleteFilter) {
        let options = {};
        if (productID!="all"){
        if (ignoreDate == true) {
          options = {
            ...
            select:"[productID]='" + productID + "' and [TranstypeDesc]='Sales Order'" ,
            LimitCount: parseInt(limitcount)||initialReportLoad,
            LimitFrom: parseInt(limitfrom)||0,
        };
        } else {
    ...
    getProductOnOrder(dateFrom, dateTo, ignoreDate,limitcount,limitfrom,productID, deleteFilter) {
        let options = {};
        if (productID!="all"){
        if (ignoreDate == true) {
          options = {
            ...
            PropertyList: "TransactionDate,ProductName,FirstColumn,SecondColumn,ThirdColumn,Qty,TotalCost,ProductID,ClassID,TransactionNo,AverageCost,Cost,Available,InStock,so,invbo,pobo,onbuild,building,Price,TotalPrice,ExtraDesc,TranstypeDesc,Deptname,Active",
            // select:"[TranstypeDesc]='Sales Order'"
            select:"[productID]='" + productID + "' and [TranstypeDesc]='Invoice'" ,
            LimitCount: parseInt(limitcount)||initialReportLoad,
            LimitFrom: parseInt(limitfrom)||0,
        };
        } else {
    ...
    getOneProductRecentTrasaction(dataSearchName, productID){
        let options = "";
        options = {
        PropertyList: "TransactionDate,ProductName,FirstColumn,SecondColumn,ThirdColumn,Qty,TotalCost,ProductID,ClassID,TransactionNo,AverageCost,Cost,Available,InStock,so,invbo,pobo,onbuild,building,Price,TotalPrice,ExtraDesc,TranstypeDesc,Deptname,Active",
        select : productID == "all" ? "" : ("([TransactionDate] f7like '" +dataSearchName +"' OR [TranstypeDesc] f7like '" +dataSearchName +"') and [productID]='" + productID + "'"),
    };
    return this.getList(this.ERPObjects.T_VS1_Report_Productmovement, options);
    }
    getOneProductOnSalesOrder(dataSearchName, productID){
        let options = "";
        options = {
        PropertyList: "TransactionDate,ProductName,FirstColumn,SecondColumn,ThirdColumn,Qty,TotalCost,ProductID,ClassID,TransactionNo,AverageCost,Cost,Available,InStock,so,invbo,pobo,onbuild,building,Price,TotalPrice,ExtraDesc,TranstypeDesc,Deptname,Active",
        select : productID == "all" ? "" : ("[TransactionDate] f7like '" +dataSearchName +"' and [TranstypeDesc]='Sales Order' and [productID]='" + productID + "'"),
    };
    return this.getList(this.ERPObjects.T_VS1_Report_Productmovement, options);
    }
    getOneProductOnOrder(dataSearchName, productID){
        let options = "";
        options = {
        PropertyList: "TransactionDate,ProductName,FirstColumn,SecondColumn,ThirdColumn,Qty,TotalCost,ProductID,ClassID,TransactionNo,AverageCost,Cost,Available,InStock,so,invbo,pobo,onbuild,building,Price,TotalPrice,ExtraDesc,TranstypeDesc,Deptname,Active",
        select : productID == "all" ? "" : ("[TransactionDate] f7like '" +dataSearchName +"' and [TranstypeDesc]='Invoice' and [productID]='" + productID + "'"),
    ...
    getProductBatchesReport(limitcount, limitfrom, deleteFilter) {
        let options = "";
    ...
    getAllBinProductVS1(limitcount, limitfrom, deleteFilter) {
        ...
        if(deleteFilter == "" || deleteFilter == false || deleteFilter == null || deleteFilter == undefined){
          if (limitcount == "All") {
            options = {
              PropertyList: "BinID, BinLocation, BinNumber, ClassName, Volume_Used, ClassID, Active",
              IgnoreDates:true,
          ...
          if (limitcount == "All") {
            options = {
              PropertyList: "BinID, BinLocation, BinNumber, ClassName, Active",
              orderby: '"BinLocation asc"',
              IgnoreDates:true,
            };
          } else {
        ...
        return this.getList(this.ERPObjects.TProductBinList, options);
      }
    ...
    getNewBinListBySearch(dataSearchName) {
        ...
        options = {
        PropertyList: "BinID, BinLocation, BinNumber, ClassName, Volume_Used, ClassID, Active",
        Search: 'Active = true and (BinLocation like "%' + dataSearchName + '%" or ClassName like "%' + dataSearchName + '%" or BinNumber like "%' + dataSearchName + '%")',
        };
        return this.getList(this.ERPObjects.TProductBinList, options);
    }
}
