...
Meteor.startup(() => {
  JsonRoutes.add("post", "/api/admin/verify/email", function (req, res) {
    jsonParser(req, res, () => {
      ...
      pool.query(query, function (error, results) {
        if (results.length == 0) {
          const query2 =
            "SELECT * FROM customers WHERE logon_name = '" + data.email + "'";
  ...
  JsonRoutes.add("post", "/api/admin/signup", function (req, res) {
    jsonParser(req, res, () => {
      const data = req.body;
      const query =
        "SELECT * FROM users WHERE email = '" + data.useremail + "'";
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length == 0) {
            const addquery =
              "INSERT INTO users (username, password, email, companyid ) VALUES ('" +
              data.username +
              "', '" +
              data.userpassword +
              "', '" +
      ...
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length != 0) {
            ...
            let _print_name_to_short_description =
              data.print_name_to_short_description ? 1 : 0;
            const updateQuery =
  ...
  JsonRoutes.add("post", "/api/updateWooCommerce", function (req, res) {
    jsonParser(req, res, () => {
      const data = req.body;
      const query =
        "SELECT * FROM clientwoocommerce WHERE id = '" + data.id + "'";
      pool.query(query, (error, results, fields) => {
        if (error) {
          return JsonRoutes.sendResult(res, {
            code: "500",
  ...
  JsonRoutes.add("post", "/api/updateAustpost", function (req, res) {
    jsonParser(req, res, () => {
      const data = req.body;
      const query =
        "SELECT * FROM clientaustraliapost WHERE id = '" + data.id + "'";
      pool.query(query, (error, results, fields) => {
        if (error) {
          return JsonRoutes.sendResult(res, {
            code: "500",
          ...
          if (results.length != 0) {
            let _enabled = data.enabled ? 1 : 0;
            const updateQuery =
              "UPDATE `clientaustraliapost` SET api_key='" +
              data.key +
              "', password='" +
              data.password +
              "', reference='" +
            ...
            let _enabled = data.enabled ? 1 : 0;
            const insertQuery =
              "INSERT INTO `clientaustraliapost` SET api_key='" +
              data.key +
              "', password='" +
              data.password +
              "', reference='" +
      ...
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length != 0) {
            let _enabled = data.enabled ? 1 : 0;
            const updateQuery =
              "UPDATE `clienttrueerp` SET user_name='" +
              data.user_name +
              "', password='" +
              data.password +
              "', `database`='" +
            ...
            let _enabled = data.enabled ? 1 : 0;
            const insertQuery =
              "INSERT INTO clienttrueerp (`id`, `user_name`, `password`, `database`, `base_url`, `enabled`, `customer_type`, `invoice_template`) VALUES ('" +
              data.id +
              "','" +
              data.user_name +
              "', '" +
  ...
  JsonRoutes.add("post", "/api/addEmployee", function (req, res) {
    jsonParser(req, res, () => {
      const data = req.body;
      const query =
        "SELECT * FROM employees WHERE employeeEmail = '" + data.email + "'";
      pool.query(query, (error, results, fields) => {
        if (error) {
          return JsonRoutes.sendResult(res, {
            code: "500",
          ...
          if (results.length == 0) {
            const addquery =
              "INSERT INTO `employees` (`employeeName`, `employeeEmail`, `title`, `firstName`, `middleName`, `lastName`, `suffix`, `phone`, `mobile`, `fax`, `skypeId`, `gender`, `note`, `username`, `password`) VALUES ('" +
              data.employeeName +
              "', '" +
              data.email +
              "', '" +
    ...
    jsonParser(req, res, () => {
      ...
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length != 0) {
            const removequery =
              "DELETE FROM `employees` WHERE (`no` = '" + data.id + "');";
    ...
    jsonParser(req, res, () => {
      ...
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length == 0) {
            const addquery =
              "INSERT INTO `customers` (`globalRef`, `name`, `email`, `country`, `address`, `phone`, `note`, `Mobile`, `citySubhurb`, `state`, `zipcode`, `accountNo`, `customerType`, `disCount`, `termName`, `firstName`, `middleName`, `lastName`, `companyName`, `fax`, `skypeID`, `website`, `logon_name`, `logon_password`) VALUES ('','" +
              data.companyName +
              "' , '" +
              data.email +
              "', '', '', '" +
  ...
  JsonRoutes.add("POST", "/api/getMCustomers", async function (req, res) {
    ...
    await axios({
      ...
      url:
        req.body.url +
        `/rest/all/V1/customers/search?searchCriteria[filter_groups][0][filters][0][field]=updated_at&searchCriteria[filter_groups][0][filters][0][value]=${lstUpdateTime}&searchCriteria[filter_groups][0][filters][0][condition_type]=gt`,
      headers: {
        Authorization: req.body.auth,
        "Content-Type": "application/json",
      },
  ...
  JsonRoutes.add("POST", "/api/getMOrders", async function (req, res) {
    await axios({
      ...
      url:
        req.body.url +
        "/rest/V1/orders?searchCriteria[filter_groups][0][filters][0][field]=updated_at&searchCriteria[filter_groups][0][filters][0][value]=" +
        req.body.lstUpdateTime +
  ...
  JsonRoutes.add("GET", "/api/getLastRanDate", function (req, res) {
    jsonParser(req, res, () => {
      const query =
        "SELECT * FROM connections WHERE id = '" + req.query.id + "'";
  ...
  JsonRoutes.add("post", "/api/updateLastRanDate", function (req, res) {
    jsonParser(req, res, () => {
      const data = req.body;
      const updateQuery =
        "UPDATE connections SET last_ran_date='" +
        data.last_ran_date +
        "' WHERE id=" +
        data.id;
      pool.query(updateQuery, (err, re, fe) => {
      ...
      pool.query(query, (error, results, fields) => {
        if (error) {
          ...
          if (results.length == 0) {
            let _enabled = data.enabled ? 1 : 0;
            const addquery =
              "INSERT INTO connections (`customer_id`, `db_name`, `account_id`, `connection_id`, `run_cycle`, `enabled`) VALUES ('" +
              data.customer_id +
              "', '" +
              data.db_name +
              "', '" +
  ...
  JsonRoutes.add("POST", "/api/getZohoCustomers", async function (req, res) {
    const lstUpdateTime = req.body.lstUpdateTime;
  ...
  JsonRoutes.add("POST", "/api/updateZohoCustomers", async function (req, res) {
      ...
      await axios({
        method: "POST",
        url: "https://www.zohoapis.com/crm/v2/Accounts/upsert",
        headers: {
